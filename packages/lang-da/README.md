...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-da.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-da)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-da.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-da)
...
`@nlpjs-neo/lang-da` is the Danish language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `da` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-da
```
...
```javascript
import { StopwordsDa } from '@nlpjs-neo/lang-da';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangDa` | the plugin that registers everything below in a container |
| `StemmerDa` | reduces a token to its stem |
| `StopwordsDa` | removes the words that carry no meaning for the classification |
| `TokenizerDa` | splits a text into tokens |
| `NormalizerDa` | lowercases the text and strips accents |
| `SentimentDa` | the sentiment dictionary of the language |
