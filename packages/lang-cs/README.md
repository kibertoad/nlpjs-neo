...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-cs.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-cs)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-cs.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-cs)
...
`@nlpjs-neo/lang-cs` is the Czech language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `cs` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-cs
```
...
```javascript
import { StopwordsCs } from '@nlpjs-neo/lang-cs';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangCs` | the plugin that registers everything below in a container |
| `StemmerCs` | reduces a token to its stem |
| `StopwordsCs` | removes the words that carry no meaning for the classification |
| `TokenizerCs` | splits a text into tokens |
| `NormalizerCs` | lowercases the text and strips accents |
| `SentimentCs` | the sentiment dictionary of the language |
