...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-sr.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-sr)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-sr.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-sr)
...
`@nlpjs-neo/lang-sr` is the Serbian language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `sr` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-sr
```
...
```javascript
import { StopwordsSr } from '@nlpjs-neo/lang-sr';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangSr` | the plugin that registers everything below in a container |
| `StemmerSr` | reduces a token to its stem |
| `StopwordsSr` | removes the words that carry no meaning for the classification |
| `TokenizerSr` | splits a text into tokens |
| `NormalizerSr` | lowercases the text and strips accents |
| `SentimentSr` | the sentiment dictionary of the language |
