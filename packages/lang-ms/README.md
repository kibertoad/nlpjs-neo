...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-ms.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ms)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-ms.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ms)
...
`@nlpjs-neo/lang-ms` is the Malay language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `ms` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-ms
```
...
```javascript
import { StopwordsMs } from '@nlpjs-neo/lang-ms';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangMs` | the plugin that registers everything below in a container |
| `StemmerMs` | reduces a token to its stem |
| `StopwordsMs` | removes the words that carry no meaning for the classification |
| `TokenizerMs` | splits a text into tokens |
| `NormalizerMs` | lowercases the text and strips accents |
| `SentimentMs` | the sentiment dictionary of the language |
