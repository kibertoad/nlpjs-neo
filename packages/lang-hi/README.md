...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-hi.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-hi)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-hi.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-hi)
...
`@nlpjs-neo/lang-hi` is the Hindi language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `hi` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-hi
```
...
```javascript
import { StopwordsHi } from '@nlpjs-neo/lang-hi';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangHi` | the plugin that registers everything below in a container |
| `StemmerHi` | reduces a token to its stem |
| `StopwordsHi` | removes the words that carry no meaning for the classification |
| `TokenizerHi` | splits a text into tokens |
| `NormalizerHi` | lowercases the text and strips accents |
| `SentimentHi` | the sentiment dictionary of the language |
