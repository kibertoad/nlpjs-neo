...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-sv.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-sv)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-sv.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-sv)
...
`@nlpjs-neo/lang-sv` is the Swedish language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `sv` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-sv
```
...
```javascript
import { StopwordsSv } from '@nlpjs-neo/lang-sv';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangSv` | the plugin that registers everything below in a container |
| `StemmerSv` | reduces a token to its stem |
| `StopwordsSv` | removes the words that carry no meaning for the classification |
| `TokenizerSv` | splits a text into tokens |
| `NormalizerSv` | lowercases the text and strips accents |
| `SentimentSv` | the sentiment dictionary of the language |
