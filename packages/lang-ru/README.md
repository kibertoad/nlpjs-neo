...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-ru.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ru)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-ru.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ru)
...
`@nlpjs-neo/lang-ru` is the Russian language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `ru` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-ru
```
...
```javascript
import { StopwordsRu } from '@nlpjs-neo/lang-ru';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangRu` | the plugin that registers everything below in a container |
| `StemmerRu` | reduces a token to its stem |
| `StopwordsRu` | removes the words that carry no meaning for the classification |
| `TokenizerRu` | splits a text into tokens |
| `NormalizerRu` | lowercases the text and strips accents |
| `SentimentRu` | the sentiment dictionary of the language |
