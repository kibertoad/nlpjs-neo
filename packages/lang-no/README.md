...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-no.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-no)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-no.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-no)
...
`@nlpjs-neo/lang-no` is the Norwegian (Bokmål) language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `no` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-no
```
...
```javascript
import { StopwordsNo } from '@nlpjs-neo/lang-no';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangNo` | the plugin that registers everything below in a container |
| `StemmerNo` | reduces a token to its stem |
| `StopwordsNo` | removes the words that carry no meaning for the classification |
| `TokenizerNo` | splits a text into tokens |
| `NormalizerNo` | lowercases the text and strips accents |
| `SentimentNo` | the sentiment dictionary of the language |
