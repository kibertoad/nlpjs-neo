...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-el.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-el)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-el.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-el)
...
`@nlpjs-neo/lang-el` is the Greek language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `el` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-el
```
...
```javascript
import { StopwordsEl } from '@nlpjs-neo/lang-el';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangEl` | the plugin that registers everything below in a container |
| `StemmerEl` | reduces a token to its stem |
| `StopwordsEl` | removes the words that carry no meaning for the classification |
| `TokenizerEl` | splits a text into tokens |
| `NormalizerEl` | lowercases the text and strips accents |
| `SentimentEl` | the sentiment dictionary of the language |
