...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-ta.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ta)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-ta.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ta)
...
`@nlpjs-neo/lang-ta` is the Tamil language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `ta` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-ta
```
...
```javascript
import { StopwordsTa } from '@nlpjs-neo/lang-ta';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangTa` | the plugin that registers everything below in a container |
| `StemmerTa` | reduces a token to its stem |
| `StopwordsTa` | removes the words that carry no meaning for the classification |
| `TokenizerTa` | splits a text into tokens |
| `NormalizerTa` | lowercases the text and strips accents |
| `SentimentTa` | the sentiment dictionary of the language |
