...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-bn.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-bn)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-bn.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-bn)
...
`@nlpjs-neo/lang-bn` is the Bengali language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `bn` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-bn
```
...
```javascript
import { TokenizerBn } from '@nlpjs-neo/lang-bn';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangBn` | the plugin that registers everything below in a container |
| `StemmerBn` | reduces a token to its stem |
| `TokenizerBn` | splits a text into tokens |
| `StopwordsBn` | removes the words that carry no meaning for the classification |
| `NormalizerBn` | lowercases the text and strips accents |
| `SentimentBn` | the sentiment dictionary of the language |
