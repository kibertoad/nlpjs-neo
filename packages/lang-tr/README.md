...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-tr.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-tr)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-tr.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-tr)
...
`@nlpjs-neo/lang-tr` is the Turkish language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `tr` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-tr
```
...
```javascript
import { StopwordsTr } from '@nlpjs-neo/lang-tr';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangTr` | the plugin that registers everything below in a container |
| `StemmerTr` | reduces a token to its stem |
| `StopwordsTr` | removes the words that carry no meaning for the classification |
| `TokenizerTr` | splits a text into tokens |
| `NormalizerTr` | lowercases the text and strips accents |
| `SentimentTr` | the sentiment dictionary of the language |
