...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-nl.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-nl)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-nl.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-nl)
...
`@nlpjs-neo/lang-nl` is the Dutch language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `nl` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-nl
```
...
```javascript
import { StopwordsNl } from '@nlpjs-neo/lang-nl';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangNl` | the plugin that registers everything below in a container |
| `StemmerNl` | reduces a token to its stem |
| `StopwordsNl` | removes the words that carry no meaning for the classification |
| `TokenizerNl` | splits a text into tokens |
| `NormalizerNl` | lowercases the text and strips accents |
| `SentimentNl` | the sentiment dictionary of the language |
