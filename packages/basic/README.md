...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/basic.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/basic)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/basic.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/basic)
...
`@nlpjs-neo/basic` is the batteries-included entry point for a backend bot: it re-exports the
container of `@nlpjs-neo/core-loader` and mounts the plugins a bot usually needs — the
logger, the evaluator and the JavaScript pipeline compiler, the NLP classes, English, and the
console connector. `dockStart()` reads `conf.json` and `pipelines.md` and returns the dock.
...
See the [quickstart](https://github.com/kibertoad/nlpjs-neo/blob/main/docs/v5/quickstart.md)
for the whole walkthrough. The rest of this page documents the container itself, which comes
from `@nlpjs-neo/core`.
...
```javascript
import { containerBootstrap } from '@nlpjs-neo/basic';
