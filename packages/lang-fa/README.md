...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-fa.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-fa)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-fa.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-fa)
...
`@nlpjs-neo/lang-fa` is the Persian (Farsi) language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `fa` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-fa
```
...
```javascript
import { StopwordsFa } from '@nlpjs-neo/lang-fa';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangFa` | the plugin that registers everything below in a container |
| `StemmerFa` | reduces a token to its stem |
| `StopwordsFa` | removes the words that carry no meaning for the classification |
| `TokenizerFa` | splits a text into tokens |
| `NormalizerFa` | lowercases the text and strips accents |
| `SentimentFa` | the sentiment dictionary of the language |
