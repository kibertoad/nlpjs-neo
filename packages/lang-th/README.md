...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-th.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-th)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-th.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-th)
...
`@nlpjs-neo/lang-th` is the Thai language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `th` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-th
```
...
```javascript
import { StopwordsTh } from '@nlpjs-neo/lang-th';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangTh` | the plugin that registers everything below in a container |
| `StemmerTh` | reduces a token to its stem |
| `StopwordsTh` | removes the words that carry no meaning for the classification |
| `TokenizerTh` | splits a text into tokens |
| `NormalizerTh` | lowercases the text and strips accents |
| `SentimentTh` | the sentiment dictionary of the language |
