...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-zh.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-zh)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-zh.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-zh)
...
`@nlpjs-neo/lang-zh` is the Chinese (Simplified) language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `zh` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-zh
```
...
```javascript
import { StopwordsZh } from '@nlpjs-neo/lang-zh';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangZh` | the plugin that registers everything below in a container |
| `StemmerZh` | reduces a token to its stem |
| `StopwordsZh` | removes the words that carry no meaning for the classification |
| `TokenizerZh` | splits a text into tokens |
| `NormalizerZh` | lowercases the text and strips accents |
| `SentimentZh` | the sentiment dictionary of the language |
| `TranslateZh` |  |
