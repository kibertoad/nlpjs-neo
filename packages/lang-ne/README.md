...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-ne.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ne)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-ne.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ne)
...
`@nlpjs-neo/lang-ne` is the Nepali language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `ne` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-ne
```
...
```javascript
import { StopwordsNe } from '@nlpjs-neo/lang-ne';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangNe` | the plugin that registers everything below in a container |
| `StemmerNe` | reduces a token to its stem |
| `StopwordsNe` | removes the words that carry no meaning for the classification |
| `TokenizerNe` | splits a text into tokens |
| `NormalizerNe` | lowercases the text and strips accents |
| `SentimentNe` | the sentiment dictionary of the language |
