...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-ro.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ro)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-ro.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ro)
...
`@nlpjs-neo/lang-ro` is the Romanian language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `ro` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-ro
```
...
```javascript
import { StopwordsRo } from '@nlpjs-neo/lang-ro';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangRo` | the plugin that registers everything below in a container |
| `StemmerRo` | reduces a token to its stem |
| `StopwordsRo` | removes the words that carry no meaning for the classification |
| `TokenizerRo` | splits a text into tokens |
| `NormalizerRo` | lowercases the text and strips accents |
| `SentimentRo` | the sentiment dictionary of the language |
