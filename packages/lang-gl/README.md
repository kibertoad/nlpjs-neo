...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-gl.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-gl)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-gl.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-gl)
...
`@nlpjs-neo/lang-gl` is the Galician language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `gl` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-gl
```
...
```javascript
import { StopwordsGl } from '@nlpjs-neo/lang-gl';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangGl` | the plugin that registers everything below in a container |
| `StemmerGl` | reduces a token to its stem |
| `StopwordsGl` | removes the words that carry no meaning for the classification |
| `TokenizerGl` | splits a text into tokens |
| `NormalizerGl` | lowercases the text and strips accents |
| `SentimentGl` | the sentiment dictionary of the language |
