...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-uk.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-uk)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-uk.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-uk)
...
`@nlpjs-neo/lang-uk` is the Ukrainian language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `uk` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-uk
```
...
```javascript
import { StopwordsUk } from '@nlpjs-neo/lang-uk';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangUk` | the plugin that registers everything below in a container |
| `StemmerUk` | reduces a token to its stem |
| `StopwordsUk` | removes the words that carry no meaning for the classification |
| `TokenizerUk` | splits a text into tokens |
| `NormalizerUk` | lowercases the text and strips accents |
| `SentimentUk` | the sentiment dictionary of the language |
