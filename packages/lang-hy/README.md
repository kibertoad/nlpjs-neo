...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-hy.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-hy)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-hy.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-hy)
...
`@nlpjs-neo/lang-hy` is the Armenian language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `hy` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-hy
```
...
```javascript
import { TokenizerHy } from '@nlpjs-neo/lang-hy';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangHy` | the plugin that registers everything below in a container |
| `StemmerHy` | reduces a token to its stem |
| `TokenizerHy` | splits a text into tokens |
| `StopwordsHy` | removes the words that carry no meaning for the classification |
| `NormalizerHy` | lowercases the text and strips accents |
| `SentimentHy` | the sentiment dictionary of the language |
