...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/core-loader.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/core-loader)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/core-loader.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/core-loader)
...
`@nlpjs-neo/core-loader` is `@nlpjs-neo/core` plus the things that need a file system: it
registers the `fs` plugin of `@nlpjs-neo/request`, resolves plugin names such as `LangEn` or
`ConsoleConnector` to their packages, and exposes `dockStart`, which loads `conf.json` and
`pipelines.md`. Use `@nlpjs-neo/core` instead when you are building for the browser.

The rest of this page documents the container, which comes from `@nlpjs-neo/core`.
