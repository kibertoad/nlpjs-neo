...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-pt.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-pt)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-pt.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-pt)
...
`@nlpjs-neo/lang-pt` is the Portuguese language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `pt` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-pt
```
...
```javascript
import { StopwordsPt } from '@nlpjs-neo/lang-pt';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangPt` | the plugin that registers everything below in a container |
| `StemmerPt` | reduces a token to its stem |
| `StopwordsPt` | removes the words that carry no meaning for the classification |
| `TokenizerPt` | splits a text into tokens |
| `NormalizerPt` | lowercases the text and strips accents |
| `SentimentPt` | the sentiment dictionary of the language |
