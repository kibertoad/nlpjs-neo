...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-ga.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ga)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-ga.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-ga)
...
`@nlpjs-neo/lang-ga` is the Irish language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `ga` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-ga
```
...
```javascript
import { StopwordsGa } from '@nlpjs-neo/lang-ga';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangGa` | the plugin that registers everything below in a container |
| `StemmerGa` | reduces a token to its stem |
| `StopwordsGa` | removes the words that carry no meaning for the classification |
| `TokenizerGa` | splits a text into tokens |
| `NormalizerGa` | lowercases the text and strips accents |
| `SentimentGa` | the sentiment dictionary of the language |
