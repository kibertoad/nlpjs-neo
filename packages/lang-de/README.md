...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-de.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-de)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-de.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-de)
...
`@nlpjs-neo/lang-de` is the German language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `de` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-de
```
...
```javascript
import { StopwordsDe } from '@nlpjs-neo/lang-de';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangDe` | the plugin that registers everything below in a container |
| `StemmerDe` | reduces a token to its stem |
| `StopwordsDe` | removes the words that carry no meaning for the classification |
| `TokenizerDe` | splits a text into tokens |
| `NormalizerDe` | lowercases the text and strips accents |
| `SentimentDe` | the sentiment dictionary of the language |
