...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-eu.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-eu)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-eu.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-eu)
...
`@nlpjs-neo/lang-eu` is the Basque language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `eu` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-eu
```
...
```javascript
import { StopwordsEu } from '@nlpjs-neo/lang-eu';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangEu` | the plugin that registers everything below in a container |
| `StemmerEu` | reduces a token to its stem |
| `StopwordsEu` | removes the words that carry no meaning for the classification |
| `TokenizerEu` | splits a text into tokens |
| `NormalizerEu` | lowercases the text and strips accents |
| `SentimentEu` | the sentiment dictionary of the language |
