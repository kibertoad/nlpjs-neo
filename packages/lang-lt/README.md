...
[![NPM version](https://img.shields.io/npm/v/@nlpjs-neo/lang-lt.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-lt)
[![NPM downloads](https://img.shields.io/npm/dm/@nlpjs-neo/lang-lt.svg?style=flat)](https://www.npmjs.com/package/@nlpjs-neo/lang-lt)
...
`@nlpjs-neo/lang-lt` is the Lithuanian language plugin of NLP.js Neo. It bundles the tokenizer, the
stemmer, the stopwords, the normalizer and, where the language has one, the sentiment
dictionary, and registers them in the container for the `lt` locale.

## Installation

```bash
pnpm add @nlpjs-neo/lang-lt
```
...
```javascript
import { StopwordsLt } from '@nlpjs-neo/lang-lt';
```
...
| Export | What it is |
| ------ | ---------- |
| `LangLt` | the plugin that registers everything below in a container |
| `StemmerLt` | reduces a token to its stem |
| `StopwordsLt` | removes the words that carry no meaning for the classification |
| `TokenizerLt` | splits a text into tokens |
| `NormalizerLt` | lowercases the text and strips accents |
| `SentimentLt` | the sentiment dictionary of the language |
