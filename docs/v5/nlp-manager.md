...
You can also provide a context to `NlpManager.process` so the NLG changes its behaviour based on the context.
...
When using entities to extract information from the user utterance the process function also generates an additional utterance by replacing all matched entity words with the relevant  entity-names and checks if this has a higher accuracy for a match then the found one. If yes then this match is used as response.

If this optional utterance was used as the better match, the key "optionalUtterance" is set in the result object returned from process method.

## Forcing NER Processing

By default entities are only processed when at least one intent has at least one entity reference. You can set the setting forceNER to true to force the NER processing.
