...
[Duckling](https://github.com/facebook/duckling) is Facebook's entity parser. It covers more
languages than the [Microsoft builtin](./builtin-entity-extraction.md), at the cost of
running a Duckling server and reaching it over HTTP.
...
You need an instance of Duckling up and running; the integration goes through its REST API.
The official page has the instructions to run it: https://github.com/facebook/duckling

Set the `ducklingUrl` property of the NER settings to use it:

```javascript
import { NlpManager } from 'node-nlp-neo';

(async () => {
  const manager = new NlpManager({ ner: { ducklingUrl: 'https://ducklinghost/parse' } });
...
Alternatively, set the `useDuckling` property of the NER to true and let the URL come from
the `DUCKLING_URL` environment variable, which defaults to `http://localhost:8000/parse`:

```javascript
import { NlpManager } from 'node-nlp-neo';

(async () => {
  const manager = new NlpManager({ forceNER: true, ner: { useDuckling: true } });
  manager.addLanguage(['en']);
  const result = await manager.process(
    'twenty five euros'
  );
...
The answer includes a property `sourceEntities` with the original response from Duckling,
and a property `entities` with the processed entities.

When you build the container yourself instead of using `NlpManager`, register the plugin as
the builtin extractor:
...
(async () => {
  const manager = new NlpManager({ languages: ['en'], forceNER: true, ner: { useDuckling: true } });
  const result = await manager.process(
