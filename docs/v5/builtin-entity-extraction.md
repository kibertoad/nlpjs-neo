...
Builtin (or "golden") entities are the ones the library recognizes without you declaring
them: emails, numbers, dates, currencies and so on. They are extracted even when the
utterance is not matched to an intent.

Three extractors are available:

| Package | What it does |
| ------- | ------------ |
| [`@nlpjs-neo/builtin-microsoft`](../../packages/builtin-microsoft) | Wraps [Microsoft Recognizers](https://github.com/microsoft/Recognizers-Text), runs in process, covers the most entity types but not every language |
| [`@nlpjs-neo/builtin-duckling`](../../packages/builtin-duckling) | Forwards to a running [Duckling](https://github.com/facebook/duckling) server, covers more languages. See [Integration with Duckling](./builtin-duckling.md) |
| [`@nlpjs-neo/builtin-default`](../../packages/builtin-default) | Regular expressions for email, URL, IP, hashtag, phone number, number, date and hex color; works in any language |

## Enabling it

`NlpManager` from `node-nlp-neo` registers the Microsoft builtin for you, so builtin
...
const result = await manager.process('en', 'My email is something@somehost.com');
console.log(result.entities);
```

Pass `ner: { ducklingUrl: '...' }` to use Duckling instead.

When you build the container yourself, register the extractor under `extract-builtin-??`,
where `??` is the locale it applies to, or `??` literally for every locale:
...
[`examples/18-ner-builtin-ms`](../../examples/18-ner-builtin-ms).

## Language support

For the languages covered by Duckling, see
[Integration with Duckling](./builtin-duckling.md#language-support).

The Microsoft builtin bundles differ per language:
